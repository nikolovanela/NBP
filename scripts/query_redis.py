...
def q9_most_active_reviewers(r, city="Philadelphia", top_n=5):
    ...
    # Count reviews per user_id across all businesses
    user_review_count = defaultdict(int)
    for raw in raw_indexes:
        if raw:
            idx = json.loads(raw)
            # We need user_id from each review — fetch a sample of reviews
            for rid in idx["review_ids"]:
                pass  # we'll do a second pipeline below
    ...
    # Batch-fetch reviews (limit to 5000 for speed)
    sample = all_review_ids[:5000]
    pipe2 = r.pipeline()
    for rid in sample:
